# early fuse, 1 auxiliary classifiers, with residual
...
def cnn_model_fn(x):
	...
	fuse_1 = tf.add(tf.add(dconv5_1,dconv9_1),dconv13_1)
	# conv5_2 = tf.layers.conv2d(inputs=fuse_1, filters=2, kernel_size=[3,3], padding="same", activation=tf.nn.relu,kernel_regularizer=regularizer)
	# conv9_2 = tf.layers.conv2d(inputs=fuse_1, filters=2, kernel_size=[3,3], padding="same", activation=tf.nn.relu,kernel_regularizer=regularizer)
	conv13_2 = tf.layers.conv2d(inputs=fuse_1, filters=2, kernel_size=[3,3], padding="same", activation=tf.nn.relu,kernel_regularizer=regularizer)
	
	# output1 = tf.layers.conv2d(inputs=conv5_2, filters=2, kernel_size=[1,1], padding="same", activation=tf.nn.relu,kernel_regularizer=regularizer)
	# output2 = tf.layers.conv2d(inputs=conv9_2, filters=2, kernel_size=[1,1], padding="same", activation=tf.nn.relu,kernel_regularizer=regularizer)
	# output3 = tf.layers.conv2d(inputs=conv13_2, filters=2, kernel_size=[1,1], padding="same", activation=tf.nn.relu,kernel_regularizer=regularizer)
	
	conv13_3 = tf.layers.conv2d(inputs=conv13_2, filters=2, kernel_size=[1,1], padding="same", kernel_regularizer=regularizer)
	output = tf.nn.relu(tf.add(fuse_1, conv13_3))
...
def train_neural_network(x):
	...
	cost = tf.losses.softmax_cross_entropy(onehot_labels=onehot_labels, logits=output)	
...
# 			lr_ = 1.0/np.power(10, epoch/100+2) # lr decay from 0.01 every 100 epochs 
		...
		pickle_out = open('eval_output_residual_earlyfuse3_ep'+str(hm_epochs)+'_lr001_wd50.pickle','wb')
		pickle.dump(eval_output,pickle_out,protocol=2)
		pickle_out.close()
		pickle_out = open('test_output_residual_earlyfuse3_ep'+str(hm_epochs)+'_lr001_wd50.pickle','wb')
		pickle.dump(test_output,pickle_out,protocol=2)
		pickle_out.close()
		save_path = saver.save(sess, '/tmp/model_residual_earlyfuse3_ep'+str(hm_epochs)+'_lr001_wd50.ckpt')
...
np.savez('loss_residual_earlyfuse3_ep'+str(hm_epochs)+'_lr001_wd50.npz',train_epoch_loss=train_epoch_loss,eval_epoch_loss=eval_epoch_loss,train_epoch_mse=train_epoch_mse,eval_epoch_mse=eval_epoch_mse,test_mse=test_mse)
