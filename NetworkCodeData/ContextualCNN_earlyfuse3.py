# early fuse, 1 auxiliary classifiers
...
def cnn_model_fn(x):
	...
	output = tf.layers.conv2d(inputs=conv13_2, filters=2, kernel_size=[1,1], padding="same", activation=tf.nn.relu,kernel_regularizer=regularizer)
...
# 			lr_ = 1.0/np.power(10, epoch/100+2) # lr decay from 0.01 every 100 epochs 
		...
		pickle_out = open('eval_output_earlyfuse3_ep'+str(hm_epochs)+'_lr001_wd50.pickle','wb')
		pickle.dump(eval_output,pickle_out,protocol=2)
		pickle_out.close()
		pickle_out = open('test_output_earlyfuse3_ep'+str(hm_epochs)+'_lr001_wd50.pickle','wb')
		pickle.dump(test_output,pickle_out,protocol=2)
		pickle_out.close()
		save_path = saver.save(sess, '/tmp/model_earlyfuse3_ep'+str(hm_epochs)+'_lr001_wd50.ckpt')
...
np.savez('loss_earlyfuse3_ep'+str(hm_epochs)+'_lr001_wd50.npz',train_epoch_loss=train_epoch_loss,eval_epoch_loss=eval_epoch_loss,train_epoch_mse=train_epoch_mse,eval_epoch_mse=eval_epoch_mse,test_mse=test_mse)
