...
def cnn_model_fn(x):
	...
	conv9_ = tf.layers.conv2d(inputs=conv8, filters=256, kernel_size=[3, 3], padding="same", kernel_regularizer=regularizer)
	conv9 = tf.nn.relu(tf.add(conv7,conv9_))
	...
	conv13_ = tf.layers.conv2d(inputs=conv12, filters=512, kernel_size=[3, 3], padding="same", kernel_regularizer=regularizer)
	conv13 = tf.nn.relu(tf.add(conv11,conv13_))
...
def train_neural_network(x):
	...
	with tf.Session() as sess:
		...
		pickle_out = open('eval_output_residual2_lr001_wd300.pickle','wb')
		pickle.dump(eval_output,pickle_out,protocol=2)
		pickle_out.close()
		pickle_out = open('test_output_residual2_lr001_wd300.pickle','wb')
		pickle.dump(test_output,pickle_out,protocol=2)
		pickle_out.close()
		save_path = saver.save(sess, "/tmp/model_residual2_lr001_wd300.ckpt")
...
np.savez('loss_residual2_lr001_wd300.npz',train_epoch_loss=train_epoch_loss,eval_epoch_loss=eval_epoch_loss,train_epoch_mse=train_epoch_mse,eval_epoch_mse=eval_epoch_mse,test_mse=test_mse)
